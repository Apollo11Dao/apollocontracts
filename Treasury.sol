...
contract ApoloTreasury is Ownable {
    ...
    address public immutable Apolo;
    ...
    address public Moon;
    uint public MoonQueue; // Delays change to Moon address
    ...
    constructor (
        ...
        require( _Apolo != address(0) );
        Apolo = _Apolo;

        isReserveToken[ _DAI ] = true;
        reserveTokens.push( _DAI );

    //    isLiquidityToken[ _APOLODAI ] = true;
    //    liquidityTokens.push( _APOLODAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for APOLO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint APOLO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Apolo ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn APOLO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAPOLO( Apolo ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( Moon ).balanceOf( msg.sender ); // Can only borrow against Moon held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with APOLO
        @param _amount uint
     */
    function repayDebtWithAPOLO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAPOLO( Apolo ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Apolo, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Apolo ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Apolo ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns APOLO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match APOLO decimals
            value_ = _amount.mul( 10 ** IERC20( Apolo ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.MOON ) { // 9
            MOONQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
