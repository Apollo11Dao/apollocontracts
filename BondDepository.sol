...
contract ApoloBondDepository is Ownable {
    ...
    address public immutable Apolo; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints APOLO when receives principle
    ...
    struct Bond {
        uint payout; // APOLO remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Apolo ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) APOLO
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Apolo ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Apolo ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Apolo ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( Apolo ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Apolo ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to APOLO supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( Apolo ).totalSupply();
    ...
    /**
     *  @notice calculate amount of APOLO available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or APOLO) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != Apolo );
